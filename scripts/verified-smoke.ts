...
// --image <file> (repeatable; sent as data URIs with --role per image, default reference_image) --model <id>
// --audio true|false and --expires <seconds> (Ark video tasks only).
...
const { values } = parseArgs({ options: {
  ...
  audio: { type: "string", default: "true" }, expires: { type: "string", default: "3600" },
} });
...
const generateAudio = values.audio !== "false";
const expiresAfter = Number(values.expires);
if (!Number.isInteger(expiresAfter) || expiresAfter < 3600 || expiresAfter > 259200) throw new Error("--expires must be an integer 3600..259200 seconds");
...
const record: Record<string, unknown> = {
  ...
  parameters: { ratio: values.ratio, resolution: values.resolution, duration, model: values.model, roles, generateAudio, expiresAfter },
...
if (values.vendor === "volcengine") {
  ...
  if (values.kind === "image") {
    ...
    const body = { model: values.model ?? "doubao-seedance-2-0-mini-260615", content: [{ type: "text", text: values.prompt }, ...imageContent], resolution: values.resolution ?? "720p", ratio: values.ratio, duration, generate_audio: generateAudio, watermark: false, execution_expires_after: expiresAfter };
    record.request = { ...redacted(body), digest: digest(body) };
