...
set -euo pipefail
cd "$(dirname "$0")/../.."
project="scenedesk-deploy-smoke-$(date +%s)-$$"
directory="$PWD/.runtime/deploy-smoke-$(date +%s)-$$"
...
status=$(curl --max-time 20 --silent --show-error --cacert "$directory/certs/ca.crt" -o /dev/null -w '%{http_code}' https://localhost:4338/design/openapi.json)
[[ "$status" == 404 ]]
