...
To retain a failed test project for diagnosis, set `SCENEDESK_SMOKE_KEEP_ON_FAILURE=1`; clean it with the exact project name and configuration paths printed/created by that run. The default always removes its containers and volumes. The browser proof page is mounted only by the smoke override, never included in the production web image; the browser ignores only the harness certificate while separate curl GET/HEAD checks verify TLS using the ephemeral CA. Its disposable profile exits with the headless command.
