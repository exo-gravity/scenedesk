...
export async function resolveVisual(
  ...
  resolved.output = {
    ...input.output,
    resolution,
    ...(kind === "video" ? { withAudio: input.output.withAudio ?? false } : {}),
  };
...
export async function assertVisualCurrent(
  tx: Transaction,
...
export async function recordVisualOrigin(
  tx: Transaction,
  planId: string,
