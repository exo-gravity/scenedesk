...
async function artifact(tx: Transaction, id: string, revision: number) {
  ...
  requireThat(
    row?.status === "succeeded" &&
      row.input.assistance?.kind === "prepare_prompt" &&
      row.resolved_input.canvasSnapshots?.length,
    409,
    "CANVAS_ASSISTANCE_UNAVAILABLE",
    "请选择此项目中已完成的固定画布建议修订。",
  );
  await assertCanvasAssistanceAccess(tx, row.resolved_input);
  for (const reference of (row.body as Schema<"AssistanceBody">)
...
function receipt(
  ...
  return {
    ...
    mode: row.mode,
    baseCanvasRevision: Number(row.base_revision),
    resultCanvasRevision: Number(row.result_revision),
...
export function canvasApplicationRoutes(
  app: FastifyInstance,
  context: ApiContext,
) {
  registerAction(
  ...
  registerAction(
    ...
    async (tx, input) => {
      const canvasId = input.params.canvasId!,
        body = input.body as Schema<"ApplyCanvasAssistance">;
      await canvasRoot(tx, canvasId, true);
      const advice = await artifact(tx, body.artifactId, body.artifactRevision),
        saved = await previous(tx, canvasId, body.applicationId),
        canvas = await readCanvas(tx, canvasId);
      if (saved) {
        requireThat(
          saved.node_id === body.nodeId.toLowerCase() &&
            saved.artifact_id === body.artifactId.toLowerCase() &&
            Number(saved.artifact_revision) === body.artifactRevision &&
            saved.mode === body.mode &&
            Number(saved.base_revision) === input.version,
          409,
          "CANVAS_ASSISTANCE_CONFLICT",
          "原应用身份已固定到另一份建议、目标或版本，请保留原意图核对。",
        );
        return {
          body: { canvas, application: receipt(saved) },
          etag: canvas.revision,
        };
      }
      ...
      const document = structuredClone(canvas.document),
        node = document.nodes.find((n) => n.id === body.nodeId.toLowerCase()),
      ...
      requireThat(
        target?.enabled &&
          ...
          target.connection_version_id ===
            advice.resolved_input.targetConnectionVersionId,
        409,
        "TARGET_CAPABILITY_CHANGED",
        "原建议的目标能力已变化，请保留草稿并核对。",
      );
      requireThat(
        node?.content.type === "draft" &&
          node.kind === target.definition.purpose &&
          node.content.capabilityId === target.id &&
          node.content.connectionId === target.connection_id,
        422,
        "CANVAS_ASSISTANCE_TARGET_MISMATCH",
        "请明确选择与此建议目标模型一致的已保存创作草稿。",
      );
      const before = node.content.prompt,
        after = safeText(
          body.mode === "replace"
            ? advice.body.prompt
            : [before, advice.body.prompt].filter(Boolean).join("\n\n"),
        );
      node.content.prompt = after;
      const result = await appendCanvas(
        tx,
        canvasId,
        document,
        canvas.revision + 1,
      ...
      return {
        body: { canvas: result, application: receipt(row) },
        etag: result.revision,
        auditObjectId: body.applicationId,
      };
    },
