...
export type EditingSessionController<T> = {
  readonly partition: EditingPartition;
  updateTransport(transport: T): void;
  getSnapshot(): { phase: string };
  pause(): void;
  retireIfDurable(): Promise<boolean>;
  suspendAccess(): void;
  refresh(): Promise<boolean>;
  revoke(options?: { preserveLocal?: boolean }): Promise<void>;
};
...
export class EditingSessionRegistry<C extends EditingSessionController<T>, T> {
  private sessions = new Map<string, { controller: C; sessionId: string }>();
  ...
  acquire(
    ...
    return this.serial(async () => {
      ...
      if (this.sessions.size >= this.limit)
        throw new EditingLocalError(
          "unavailable",
          "已有多份编辑仍在保存或未能保留到本机。请先返回这些编辑页处理保存问题，再打开新的编辑页；现有输入仍保留。",
        );
  ...
  release(key: string, controller: C) {
    controller.pause();
    return this.serial(async () => {
  ...
  private matching(hint: EditingAccessHint) {
    return [...this.sessions.values()].filter(({ controller, sessionId }) => {
      ...
      return (
        ...
        (hint.kind === "session" ||
          ((p.kind === "canvas" ? "canvas" : "cut") === hint.kind &&
