...
export function CanvasImageGeneration({
  tenantId,
  projectId,
  sceneId,
  controller,
  ...
  const images =
    entries.data?.filter((entry) => entry.plan.input.purpose === "image") ?? [];
  ...
  const nodeId =
    ...
    (selected?.kind === "image" && selected.content.type === "draft"
      ? selected.id
      : undefined);
            ...
            <NativeSelect
              ...
              data={[
                ...
                ...images.map((entry, index) => ({
                  value: entry.plan.id,
                  label: `${snapshot.local?.document.nodes.find((node) => node.id === entry.origin.nodeId)?.title ?? "已删除的图片草稿"} · ${index + 1} · ${entry.jobId ? "已有任务" : "固定计划"}`,
                })),
              ]}
            />
            {nodeId ? (
              <ImageGenerationWorkspace
                ...
                source={{
                  ...
                  configure: (id, change) => {
                    ...
                    if (node?.kind !== "image" || node.content.type !== "draft")
                      throw Error("原图片草稿已改变，请重新选择。");
                    controller.change({
              ...
              <Text size="sm" c="dimmed">
                先选择图片草稿。文字和已有图片可以通过“继续创作”创建独立图片草稿。
              </Text>
            )}
          </Stack>
        </Accordion.Panel>
