...
export function CanvasMediaGeneration({
  ...
  return (
    <section className={classes.canvasGeneration} aria-label="画布生成与结果">
      <Group
      ...
      >
        <Group gap={6}>
          <Symbol size={15} />
          <Text size="xs" fw={600}>
            {history ? "查看固定任务" : nodeId ? `${label}创作` : "生成记录"}
          </Text>
        </Group>
        {historyId && (
          <Button size="xs" variant="subtle" onClick={() => setHistoryId(null)}>
            返回当前草稿
          </Button>
        )}
      </Group>
      <Stack gap="sm">
        ...
        />
        <details className={classes.historyPicker}>
          <summary>
            生成记录 <span>{images.length} 项</span>
          </summary>
