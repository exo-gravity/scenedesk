-- Every saved advice reference retains typed identities; additions require current authority.
ALTER TABLE assistance_revision_refs ADD COLUMN subject_asset_id uuid;
ALTER TABLE assistance_revision_refs ADD FOREIGN KEY(tenant_id,subject_asset_id) REFERENCES assets(tenant_id,id);
...
BEGIN
 ...
 FOR reference IN SELECT value FROM jsonb_array_elements(NEW.body->'referenceSuggestions') LOOP
  ...
  SELECT EXISTS(SELECT 1 FROM assistance_artifact_revisions r CROSS JOIN LATERAL jsonb_array_elements(r.body->'referenceSuggestions') old WHERE r.artifact_id=NEW.artifact_id AND r.number=NEW.number-1 AND old-'note'=reference-'note') INTO kept;
