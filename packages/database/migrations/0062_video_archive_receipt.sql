...
BEGIN
 ...
 IF p.input->>'purpose' NOT IN ('image','video') THEN PERFORM finish_text_assistance_job(wanted,evidence_id,output,failure);RETURN;END IF;
 ...
 media_kind:=p.input->>'purpose';collection:=CASE media_kind WHEN 'video' THEN 'videos' ELSE 'images' END;
 ...
 file_name:=CASE image_source->>'mime' WHEN 'video/mp4' THEN 'generation.mp4' WHEN 'image/png' THEN 'generation.png' WHEN 'image/jpeg' THEN 'generation.jpg' ELSE 'generation.webp' END;
 INSERT INTO media(id,tenant_id,project_id,scope,kind,status,display_name,safe_original_file_name,created_by,source_job_id,mime) VALUES(j.id,j.tenant_id,j.project_id,'project',media_kind,'processing',CASE media_kind WHEN 'video' THEN '生成视频（显式技术测试）' ELSE '生成图片（显式技术测试）' END,file_name,j.created_by,j.id,image_source->>'mime');
