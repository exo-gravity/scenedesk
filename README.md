...
2026-09-10 更新：用户已授权正式工程实施与 GitHub 合入，当前开始身份、权限和项目基础切片。先以导入素材及模拟供应商推进，无真实模型也可实施制作、剪辑、审阅与交付。完成情况和实际验收以[实施进度](docs/implementation/22-implementation-progress.md)为准；下文原型和 S0 说明仍描述现有交付，不能视为完整 MVP 已完成。
...
当前已实现：**真实身份／权限／项目基础**，包括工作室、成员邀请、所有权交接、项目与负责人、剧目设定、归档恢复；有 PostgreSQL 持久化、RLS、CAS、幂等与对应业务页面。[实现与运行说明](docs/implementation/23-identity-project-foundation.md)列出边界。集场镜、媒体、生成、编辑渲染和交付仍在后续实施中，完整 MVP 尚未完成。
...
`setup:business` 执行迁移、创建独立数据库角色，并把随机本地凭据写入权限为 0600、Git 忽略的 `.env.business`；只需首次运行，再启动直接使用 `dev:business`。已有配置不会覆盖。业务 API 位于 `127.0.0.1:4310`，身份模拟器位于 `127.0.0.1:4320`。`/health/ready` 只证明当前身份与项目基础可运行，`completeMvp` 仍为 false。
...
| 位置 | 当前职责 |
|---|---|
| `apps/web` | React/Vite 真实工作室／项目基础页面与独立的设计参照；Query 按用户与资源范围缓存 |
| `apps/api` | Fastify 业务内核、OIDC、工作室／成员／项目与剧目接口 |
