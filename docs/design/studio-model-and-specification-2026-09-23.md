...
日期：2026-09-23。状态：**已确认**（用户 2026-09-23 逐条确认：数据来源、命名、模式独立成胶囊、比例与清晰度取值）。真实供应商接入（PR #74–#77）之后，底部三个控件显示的内容已经不合用：模型名是原始 profile id，同一模型重复出现，清晰度是拍平的像素串。本决定只改这些控件的**内容与数据来源**，不动 [核心创作区重建](creative-workspace-rebuild-libtv-2026-09-21.md) 定下的布局、密度与交互，也不动生成、幂等、权限、恢复的任何行为。
...
- 换模式会改变可挂参考的用途与张数，**但参考区并不按能力记录渲染**——`apps/web` 至今不读 `supportedPurposes`／`inputRules`／`maxReferences`，`ComposerReferences` 无条件列出全部用途。因此挂着参考图用途的参考再切到首尾帧，提交会被接口以 422 `IMAGE_REFERENCE_UNSUPPORTED` 拒绝，界面上没有预兆。这在本次之前就存在（两种模式原本是模型列表里的两行，切换效果相同），胶囊只是让它更容易撞上。**本次不修**：按用途过滤参考属于参考区，不属于底栏这一片；留作后续片，届时要决定是换模式时丢弃不兼容的参考，还是在面板上提示。
