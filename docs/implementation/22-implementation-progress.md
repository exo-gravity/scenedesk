...
2026-09-16 合入后本机整体验证：同一产品代码的完整数据库回归 **327/327 通过，零跳过／失败**（48 个顶层用例，547 秒）；没有将其等同于自动浏览器回归或完整媒体通过。持久化本地工作台已升级到 `126d8a8`：先备份原 `drama` schema，再顺序应用 0106／0110／0111／0115 并验证受限角色，保留 2 个项目、2 份剧本修订、2 个场次、1 个画布与 13 份画布修订。生产构建浏览器确认原登录、正文、旧场次画布、6 个已选用镜头和实际视频播放有效。原 4343 一次性预览继续保留，常用入口为本机 4311；真实飞书配置、真实模型、25 条自动 E2E 和完整媒体／部署验收仍未关闭。证据位置见[70](70-creative-workspace-refactor.md)。
