...
## 按影响范围执行验证（2026-09-25，待合并）

[PR #98](https://github.com/exo-gravity/scenedesk/pull/98)，代码提交 `e8469b9447f10d2505655b9cfbae0e17c6ad400d`。本地 preflight 与通用、浏览器、部署 CI 共用一份范围规则：Markdown／文档图片／证据只检查文档；Web 变更执行根检查与生产浏览器回归；媒体、Worker、API、部署按受影响链路追加；数据库、契约、共享夹具、依赖锁及未知路径保守执行全套。完整矩阵与选项见[根说明的检查部分](../../README.md#检查)。
...
本片未改业务实现，未运行数据库、完整媒体、浏览器、部署或恢复套件；这些套件仍由相关范围和显式全量入口执行。既有 MinIO 镜像拉取授权故障未修复。上述本地通过不代表远端 CI、合并或部署已完成。
