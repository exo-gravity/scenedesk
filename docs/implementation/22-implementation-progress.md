...
2026-09-16 导航与场次目录细化：按用户批准的三页效果图，统一项目／场次画布切换，把场次目录移入画布菜单；剧本主操作改为明确选文引用，分镜建议下沉。轻量目录、详情抽屉、基础新建、旧链接和草稿保留已完成。最终 265/265 基础检查、6/6 索引数据库专项及 **36/36 生产浏览器用例通过**；完成同尺寸设计对照和 820px／390px 检查。重复原文的精确键盘选择与快速缩屏后的导航激活均已修复。具体证据、审查与本机切换见 [78](78-canvas-navigation-directory.md)；本次不扩大真实模型、团队飞书与外部部署的验收结论。
