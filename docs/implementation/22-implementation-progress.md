...
| G01–G04 生成与费用 | 实施中 | [固定分镜建议](41-generation-assistance.md)的后端和助手页面已提交并进入整合，包含固定计划、持久作业／attempt、未知提交恢复、人工修订与采纳；实际 API／受限 worker 的回包丢失、刷新找回、人工修订和明确采纳已通过生产浏览器验证，完整整合检查 60 项单元及 160 项数据库测试通过；当前切片远端 CI／合并待完成。提示准备与画布浏览器性能继续使用独立 agent 工作区并行推进。无模型配置时明确不可用，测试适配器不算真实生成；商业计费后置 |
