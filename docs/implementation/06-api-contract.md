...
项目创建补充：携带 `CreateProject.creationRequestId` 时，HTTP 键及输入冲突规则仍适用；相同创建用户、工作室和创建身份另外持久绑定一个项目，回执到期后也不重复创建。合法恢复返回该项目的当前表示和版本，不回放旧名称或旧归档状态。新 HTTP 键或回执过期后，创建身份与原输入不一致返回 `PROJECT_CREATION_CONFLICT`；尚有效的同 HTTP 键改输入仍优先返回 `IDEMPOTENCY_CONFLICT`。旧客户端不携带此字段时保持原 24 小时回执协议，详见 [56](56-project-creation-recovery.md)。
