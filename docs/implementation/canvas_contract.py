...
def schemas():
    ...
    s["CanvasTextContent"] = obj({"type": enum("text"), "text": TEXT, "sourceExcerpt": ref("ScriptExcerpt")}, ["type", "text"])
    s["PlaceCanvasScriptExcerpt"] = obj({"nodeId": ID, "sourceExcerpt": ref("ScriptExcerpt"), "position": ref("CanvasPoint")}, ["nodeId", "sourceExcerpt", "position"])
    s["CanvasScriptExcerptReceipt"] = obj({"canvasId": ID, "nodeId": ID, "sourceExcerpt": ref("ScriptExcerpt"), "nodeActive": BOOL}, ["canvasId", "nodeId", "sourceExcerpt", "nodeActive"])
...
def register_routes(route, paths):
    ...
    route("post", canvas + "/script-excerpts", "placeCanvasScriptExcerpt", "PR-17", "明确添加固定剧本选文；同一节点身份不重复添加", "CanvasScriptExcerptReceipt", "PlaceCanvasScriptExcerpt", code=200, cas=True)
    route("get", canvas + "/script-excerpts/{nodeId}", "getCanvasScriptExcerpt", "PR-17", "核对固定选文回执，节点移除后仍可查询", "CanvasScriptExcerptReceipt")
