...
| 多选后的批量生成 | 多选只提供“查看本次所选节点的生成计划”这一**导航**动作，不直接提交任何付费任务。批次确认屏逐项列出模型能力、解析后的输入、阻断原因、计划身份与合计预留（当前适配器恒为零并注明），计划带明确有效期；**只有用户在屏内显式确认后**才提交，且不自动采用、不自动进候选。提交后成功的项保留，重试只针对失败与过期项，不重跑整批。批次只是既有固定计划／作业的分组，不是第二套调度 |
...
根据[已确认重构方向](../design/creative-workspace-approved-2026-09-16.md)，`/canvas` 是正式项目画布工作区，无需先创建集／场／镜。使用现有 CanvasController、共同画布文档、固定生成计划、助手会话和恢复流程；旧 SceneCanvasSession 在没有场次参数时不挂载分镜台、场次助手或镜头绑定。已有场次画布由按需入口继续访问，身份不变。

唯一项目画布通过 `project_canvas_links` 建立；创建事务先按项目串行，再插入 Canvas 与第一修订及唯一关系，同事务产生 Canvas outbox。新的画布生成入口与场次入口共用固定计划处理；SQL 和服务端的草稿上下文、显式节点上下文、助手讨论作用域都验证真实项目／场次关系、活动父级与当前权限。孤立 Canvas 不能充当有效生成来源；固定旧场次 scope 不改写。

项目左侧导航、账号菜单中的链接和已有场次跳转，共用当前 CanvasSession 的本机输入／生成草稿保留、助手保留与个人偏好 flush 屏障；失败留在原页并提供明确重试。全局注销仍走原会话退出屏障。画布在私人偏好中始终为 canvas 模式，个人浏览位置可以在归档后继续保存，不构成业务修改。实施与验收见 [71](71-project-canvas-workspace.md)。
