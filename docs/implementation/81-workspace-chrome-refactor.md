...
| 剧本操作行 | 「选文带入画布」保留常驻；取消下面那行常驻提示文字。**（原决定中的「未选中正文时 `disabled`」经核实前提有误，未采用，理由见切片 3）** |
| 剧本页菜单 | 从「…」移除「场次目录」；场次目录统一由画布菜单提供；`.../content` 旧深链继续可达 |
| 侧栏图标 | 折叠改用面板图标 `SidebarSimple` 并补 Tooltip；返回保留 `ArrowLeft`；两者尺寸统一。**（原决定写的是「统一到主题 token」，因主题中无图标尺寸 token 而未采用，理由见切片 2）** |
...
| 完整生产浏览器套件 `npm run test:e2e`（一次性 `drama_e2e_controls` 库，真实 API） | **38/38 通过**，2.4 分钟（含本片新增的 `canvas-groups.spec.ts`） |
...
分支 `feat/script-page-behaviour`。

**改动。** 剧本页「…」菜单移除「场次目录」项，只剩三项页内动作（分镜建议、提案历史与导入、核对创作依据）；场次目录统一由画布菜单提供。剧本操作行取消常驻提示行，把说明移进「选文带入画布」按钮的 Tooltip。

`.../content` 旧深链未受影响，仍有六处入口可达：画布切换器（`SceneNavigator.tsx:345`，`ProjectCanvasEntry.tsx:269` 与 `SceneProductionWorkspace.tsx:305` 打开它；已批准设计指定的归属）、项目页主按钮（`Projects.tsx:293`）、项目创建后（`Projects.tsx:175`）、占位页返回（`BusinessApp.tsx:815`）、各处的「返回场次目录」（`SceneProductionWorkspace.tsx:246/266/280`）、以及镜头与剪辑工作区（`CandidateWorkspace.tsx:146`、`CutWorkspace.tsx:111`）。`tests/e2e/workspace.spec.ts:67` 断言切换器会落到 `/content`。

**一处需要纠正的事实。** 确认决定写的是「未选中正文时 disabled 并给 Tooltip」，其前提是「该动作依赖文本选中」。核实 `ScriptCanvasExcerpt.tsx` 后此前提不成立：该按钮是**弹窗开启器**（点击 `setOpened(true)`），弹窗内自带可拖选的原文本域，并写着「在原文中选中一段，作为画布参考继续创作」。按原决定实现会**砍掉一条真实可用的路径**，还会让 `tests/e2e/script-canvas.spec.ts` 里四处「先在弹窗内选文」的点击超时。因此改为只做对齐修复：提示行移进按钮 Tooltip，按钮保持可用。

需要如实说明的代价：**对齐效果与功能路径都不受影响，但信息可见性受影响**。原先那行提示对**只看不点**的用户是常驻可见的；现在它是 Tooltip，指针悬停或键盘聚焦才出现，而且措辞同时覆盖两条路径（「可在正文中选中，或打开后从原文中选择」）。弹窗内的等价说明仍在。
...
|---|---|
| `npm run check` | 265/265 通过 |
| 完整生产浏览器套件 `npm run test:e2e`（一次性 `drama_e2e_scriptbehaviour` 库，真实 API） | **38/38 通过**，2.4 分钟 |
| 真实工作台 UI 验证（本机 4311，真实 API 与数据，只读取与测量） | **10/10 通过** |

对齐用包围盒测量，并做了反事实对照：**修复前**「选文带入画布」的纵向中心为 244，同排其余三个控件为 255（**高 11px**，即被报告的现象）；**修复后**四个控件均为 244。
