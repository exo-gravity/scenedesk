...
分支 `feat/page-header-grammar`。

**范围。** 效果图第 01 节只画了**资产库页**。需要说明的是：媒体视图与本页共用同一个 `LibraryNavigation` 组件（`AssetWorkspace.tsx` 与 `MediaWorkspace.tsx` 都渲染它），所以这次的三带页头与 9 项分类行**同时作用于媒体视图**，只是「筛选」在该视图里仍位于标题行。真正不同构的是「我的工作」页（它用自己的 `SegmentedControl`），未在本片改动。把媒体视图的「筛选」也移到查询行、以及我的工作页是否对齐，都需要先出对照效果图再定，见文末。

**改动。** `libraryCategories` 的顺序改为与契约的两个 kind 枚举一致：设定资产 5（角色／场景／道具／风格／声音设定）｜ 媒体素材 4（图片／视频／音频／文本）。**移除 `slice(0, 7)` 与「更多」菜单**，9 项全部铺开；分割线仍落在两组之间（`data-divider` 在「图片」上，重排后位置正确）。`.categories` 补上该文件的第一条媒体查询：900px 以下改为 `flex-wrap: nowrap` + 横向滚动，视图行高度保持单行。死规则 `.more` 一并移除。
...
|---|---|
| `npm run check` | 265/265 通过 |
| 完整生产浏览器套件 `npm run test:e2e`（一次性 `drama_e2e_s4` 库，真实 API） | **38/38 通过**，2.5 分钟 |
| 真实工作台 UI 验证（本机 4311，真实 API 与数据，只读取与测量） | **12/12 通过** |

分类与断点都用实测，并做了反事实对照：

- 分类：实测顺序为「角色 场景 道具 风格 声音设定 图片 视频 音频 文本」，共 9 项，`nav` 内无「更多」，`[data-divider]` 落在「图片」。
- 三带：身份行 y=90、视图行 y=176、查询行 y=238；主操作在视图行之上，筛选与搜索同行。
- 断点：390px 时内容宽 586 > 可视宽 298，横向滚动，单行 47px；820px 时单行 47px 且无溢出。**反事实对照**：临时移除该媒体查询后，390px 下分类行高度变为 **154px**（折成多行），`flex-wrap` 不再是 `nowrap`；`wrap` 模式下真正的折行点在约 730px。900px 这个阈值比折行点保守约 170px，选它是为了与 `ProjectNavigation` 既有的 900px 窄屏断点一致。

**审查发现并已修的问题。**

1. **（High）已批准设计文档被违反却未标注。** `docs/design/unified-asset-library-2026-09-15.md` 的「已接受方向」第二条明确写着「低频文本与声音设定在‘更多’。窄屏允许分类自然换行，不能挤窄文字或产生页面横向滚动」——本片三点全部相反。虽经 2026-09-17 评审确认，但那份设计记录被留成了与实现相反的状态。已按仓库「被替代材料在首部标注状态并指向当前入口」的约定，在该文档首部标注取代关系并指向本文件，原文保留。
2. **（High）横向滚动容器裁掉了焦点环。** `.category:focus-visible` 的 `outline-offset: 3px` 把 2px 的环画在滚动容器之外，900px 以下每个分类的环在上、下、右侧都被裁掉；位于 `scrollMax` 的「文本」有 5px 永久不可见。已把 `outline-offset` 改为 `-2px`（环画在控件内），不再被裁。**注意这条覆盖必须写在基础 `.category:focus-visible` 之后**：两者特异性相同（0,2,0），第一次写在文件靠前位置时被静默盖掉，实测仍为 `3px`；移到基础规则之后才生效（窄屏 `-2px`、宽屏 `3px`）。
3. **（Medium）窄屏下选中的分类可能在视野外。** 旧的「更多」触发器会显示选中项名称，删除后 9 项里靠后的 4 项在 390px 首屏看不到任何高亮。已加 `useEffect`：分类变化时只调整该导航容器自身的 `scrollLeft` 把 `[aria-current="page"]` 滚入视野，不触发页面滚动。
4. **（Low）注释过度声称。** 原注释写「与契约的两个 kind 枚举一致」，而组内顺序沿用改动前次序，并非枚举书写顺序。注释已改为陈述分组关系。

**待确认（不在本片）。** 是否把媒体视图的「筛选」也移到查询行右端、以及「我的工作」页是否对齐同一页头语法，需要先出对照效果图再定。

**四片的最终状态。** 切片 1／2／3／4 分别经 [PR #58](https://github.com/exo-gravity/scenedesk/pull/58)／[#59](https://github.com/exo-gravity/scenedesk/pull/59)／[#60](https://github.com/exo-gravity/scenedesk/pull/60) 与本片合入。每片都经过了项目检查、无对话上下文的独立 agent 对抗式审查、以及真实浏览器 + 真实 API 的端到端验证。
