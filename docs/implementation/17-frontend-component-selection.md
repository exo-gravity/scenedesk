...
React Flow 12.11.6 已随场次制作路由进入主应用，复用现有 Mantine、Query 和 Media Chrome；领域文档与引擎状态分离。双模式保存／恢复、真实 API 冲突、导入视频单播放器释放及桌面布局已取得[本地验证](39-scene-canvas.md)。此前隔离选型记录仍按当时边界保留；本次不代表 300／2,000 节点浏览器容量、完整镜头绑定或 AI 生成验收完成。
