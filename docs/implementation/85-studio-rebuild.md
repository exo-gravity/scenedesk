...
本节保留交付时的实现记录；当前呈现与来源定位已由 §1o 替代。
...
## 1n. 新增镜头精简（2026-09-24）

新增镜头只负责建立本场的一个镜头条目。完整镜头要求仍在场次目录的「镜头要求与历史」中编辑；新建时不要求填写机位、连续性、参考、台词与固定摘录，也不引入分步向导或新的状态。

| 行为 | 实现与验收边界 |
|---|---|
| 最小输入 | Studio 与场次目录共用 `StructureEditor` 的简短新建表单：镜头名称必填，镜头说明选填，所属场次显示为继承的上下文；首次打开聚焦名称，创建与取消在可视范围内 |
| 创建后定位 | Studio 中，成功回执与本机草稿清理一致后关闭表单，聚焦返回的镜头 ID；内容树尚未读到新镜头时显示读取状态，不显示旧镜头。场次目录沿用原有关闭编辑面板的行为 |
| 本机恢复 | 取消先保留输入；Studio 关闭前沿用本机保留屏障，刷新后可恢复。提交回执丢失时显式恢复原创建请求与幂等键，不生成第二个镜头；场次目录的关闭按钮沿用原自动保存行为 |
| 旧草稿兼容 | 旧草稿含完整镜头要求或不同所属场次时，展开原表单供核对并按原要求保存；Studio 创建后定位到它实际所属的场次 |
| 离开后的回执 | 关闭新建面板时立即卸载编辑器；迟到回执仍完成草稿清理，但不重新打开镜头抽屉，也不把用户带回离开的场次 |

回归规格为 `tests/e2e/studio-shot-creation.spec.ts` 的六个用例。新增表单在 1366×768、820×900、390×844 检查字段与操作可见且无需外层滚动；这不代表整个 Studio 已完成移动端适配。候选、明确选用、完整要求编辑、公开契约与数据库结构保持原有语义。实际运行与合入结果见[实施进度](22-implementation-progress.md)。

生产构建的合成数据截图：[桌面新建](../design/assets/2026-09-24-shot-creation/shot-create-1366.png)、[窄屏新建](../design/assets/2026-09-24-shot-creation/shot-create-390.png)。

## 1o. 镜头整理连续浏览（2026-09-24）

本节替代 §1g 的表格与抽屉呈现，新增镜头沿用 §1n。职责与交互决定见[镜头整理设计](../design/shot-organizer-2026-09-24.md)。

| 项目 | 当前行为 | 验证 |
|---|---|---|
| 连续查看 | 桌面左侧视觉列表独立滚动，点击整行直接切换右侧详情；名称、说明、候选及明确选用状态可扫读，素材文件名留在预览旁 | 12 镜头滚动、延迟读取、排序后焦点仍属于同一镜头 |
| 预览与比较 | 有界预览完整显示画幅，保留候选与交付动作；按实际预览区宽度并排或上下比较 | 1366／820／390px 生产页面，真实播放器控制条与几何边界断言 |
| 来源定位 | 跟随当前预览候选，点击时重新检查本场与项目创作台，优先定位本场节点 | 无项目画布但有场次节点可定位；素材移除就地说明；网络失败可重试 |
| 导航与恢复 | 切镜、离场、打开其他操作后旧定位请求失效；切场前记录当前镜头，后退恢复该镜头；手机返回列表保留滚动位置并停止隐藏播放 | 迟到响应、返回／前进、移动端卸载播放器、草稿与跨场次新建回归 |
| 业务事实 | 原有候选、明确采用、历史与原片交付继续复用；列表从内容根的选用 ID 匹配候选 | 预览其他候选不改采用；单镜原片 SHA 与本场选用包顺序、清单一致；并发与撤权保护 |

没有增加筛选体系、布局切换、拖拽框架、后台接口或新的业务状态。列表仍按镜头读取候选及预览素材，未做批量接口或虚拟化。

生产浏览器完整运行 `20260924-shot-organizer-release` 为 38/38 通过、零重试；新增镜头六项和镜头整理八项均包含其中。截图使用明确的合成视频，仅证明交互与几何行为，不构成真实模型验收。独立审查核对实际 diff 与窄屏比较截图，无剩余阻断项。完整执行与交付状态见[实施进度](22-implementation-progress.md)。
...
| `studio/shots/` | `ShotsView.tsx`：视觉镜头列表、直接预览中的候选与选用、来源定位、交付、顺序、新增 | ⑦ |
