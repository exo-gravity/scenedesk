...
function Check() {
  ...
  return <MantineProvider theme={theme, cssVariablesResolver}><Stack maw={960} mx="auto" p="md"><Text>受控测试：两个实际 MediaPlayer 实例，不是模型输出。</Text><Text aria-label="播放器错误数">{errors}</Text><Button onClick={()=>setMounted(value=>!value)}>切换播放器挂载</Button>{mounted && ["A","B"].map(id=><MediaPlayer key={id} src="/__controlled__/video.mp4" title={`受控播放器 ${id}`} audio={false} onError={()=>setErrors(value=>value+1)} onTime={()=>{}} />)}</Stack></MantineProvider>;
}
