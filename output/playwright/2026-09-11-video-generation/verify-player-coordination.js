async (page) => {
  ...
  await page.getByText('受控测试：两个实际 MediaPlayer 实例，不是模型输出。',{exact:true}).click();
  const result=await page.locator('video').evaluateAll(async elements=>{
    ...
    return {actualInstances:2,decoded:true,playExclusive:true,mutedPlaybackPreserved:true,unmuteExclusive:true,zeroVolumePreserved:true,volumeIncreaseExclusive:true};
  });
  await page.screenshot({path:'output/playwright/2026-09-11-video-generation/two-players.png',fullPage:true,animations:'disabled'});
