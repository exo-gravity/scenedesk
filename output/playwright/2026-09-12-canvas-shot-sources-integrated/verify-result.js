...
,jobId="73235f8a-e573-4a4c-9dd7-605cc10fc7f5",planId="91f7ba24-f520-4425-928c-b35d0af0f6da",out='output/playwright/2026-09-12-canvas-shot-sources-integrated';
 ...
 const session=await (await page.request.get(`${f.origin}/v1/session`)).json();const grantResponse=await page.request.post(`${base}/media/${media.id}/access`,{headers:{origin:f.origin,'x-csrf-token':session.csrfToken,'idempotency-key':await page.evaluate(()=>crypto.randomUUID())},data:{variant:'original',disposition:'attachment'}});ensure(grantResponse.ok(),`original access failed ${grantResponse.status()}: ${await grantResponse.text()}`);const grant=await grantResponse.json();const download=await page.request.get(grant.url);ensure(download.ok(),'original download failed');const bytes=await download.body();const sha256=await page.evaluate(async bytes=>Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',new Uint8Array(bytes)))).map(v=>v.toString(16).padStart(2,'0')).join(''),Array.from(bytes));ensure(sha256===f.expectedOutputSha256,'downloaded bytes differ');
