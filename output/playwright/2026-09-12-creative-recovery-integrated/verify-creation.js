async (page) => {
  ...
  const base = `/v1/tenants/${tenant}`,
    ...
    app = `http://127.0.0.1:4316/#/app/t/${tenant}/p/${projectId}/content`;
  const identity = {
    id: sessionId,
    revision: 1,
    userId,
  ...
  await page.goto(app);
  await page.getByText("已有场次", { exact: true }).first().waitFor();
  ...
  for (const kind of ["episode", "scene", "shot"]) {
    ...
    await dialog.getByText("创建结果待确认", {exact: true}).scrollIntoViewIfNeeded();
    await page.screenshot({
      path: `output/playwright/2026-09-12-creative-recovery-integrated/${kind}-unknown-restored.png`,
      animations: "disabled",
    });
    await dialog
  ...
  await dialog
    ...
    .waitFor();
  await page.screenshot({
    path: "output/playwright/2026-09-12-creative-recovery-integrated/committed-cleanup-failed.png",
    animations: "disabled",
  });
  await dialog.getByRole("button", {name: "重试清理本地草稿", exact: true}).click();
  await dialog.getByRole("button", {name: "重试清理本地草稿", exact: true}).waitFor();
  if (attempts.length !== beforeCleanup + 1) throw Error("Cleanup retry wrote business data");
  await page.reload();
  await page.getByText("成功后的清理恢复", { exact: true }).first().waitFor();
  dialog = await open("scene");
