async(page)=>{
 ...
 await page.goto(`http://127.0.0.1:4318/#/app/t/${data.tenantId}/p/${data.projectId}/production?scene=${data.sceneId}&mode=canvas`);await page.reload();await page.locator('.react-flow__node').first().waitFor();
 ...
 for(const kind of ['pan','zoom'])for(let run=0;run<3;run++){
 ...
 return {case:__CASE__.nodeCount,phase:'optimized',results};
}
