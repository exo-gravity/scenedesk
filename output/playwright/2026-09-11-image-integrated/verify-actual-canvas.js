async (page) => {
  ...
  try {
    ...
    if (existing)
      ...
      await page.getByRole("button", { name: "图片草稿", exact: true }).click();
    await page
    ...
    await page
      ...
      .fill(prompt);
    await page
      .getByRole("button", { name: "画布图片生成与结果", exact: true })
      .click();
    ...
    if (
      ...
      entry.plan.resolvedInput.output.resolution !== "256x256"
    )
    ...
    await panel
      .getByRole("button", { name: "定位图片结果", exact: true })
      .click();
    const resultNode = page.locator(`.react-flow__node[data-id="${resultId}"]`);
    await resultNode.locator("img").waitFor();
    await resultNode.locator("img").evaluate((image) => image.decode());
    await page.getByRole("button", { name: "适应内容", exact: true }).click();
    await page.screenshot({
      path: "output/playwright/2026-09-11-image-integrated/canvas-result.png",
      animations: "disabled",
    });
    ...
    await page
      ...
      .click();
    await page
      .getByRole("combobox", { name: "画布图片任务历史", exact: true })
      .selectOption(entry.plan.id);
    await panel
      .getByRole("button", { name: "打开所选的固定图片任务", exact: true })
      .click();
    await panel.locator("img").waitFor();
    await panel.locator("img").evaluate((image) => image.decode());
    await panel.locator("img").scrollIntoViewIfNeeded();
    await page.screenshot({
      path: "output/playwright/2026-09-11-image-integrated/deleted-source-history.png",
      animations: "disabled",
    });
