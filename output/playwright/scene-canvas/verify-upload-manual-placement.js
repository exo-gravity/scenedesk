async (page) => {
  ...
  for (const [width, height] of [
    ...
    await page.screenshot({
      path: `output/playwright/scene-canvas/16-upload-recovery-${width}.png`,
    });
    const layout = await page.evaluate(() => ({
      width: innerWidth,
      horizontalOverflow: document.documentElement.scrollWidth > innerWidth,
    }));
    if (layout.horizontalOverflow) throw Error("Layout overflow");
    layouts.push(layout);
  }
  await page.setViewportSize({ width: 1512, height: 982 });
  await row.getByRole("button", { name: "放入画布", exact: true }).click();
  const node = await poll(async () => {
    const s = await read();
    return s.canvas.document.nodes.find((n) => n.id === entry.nodeId);
  }, "Manual placement not saved");
  if (
    node.content.mediaId !== entry.upload.mediaId ||
    completeCount !== 1 ||
    errors.length
  )
    throw Error("Fixed media recovery differs");
  if (failedReads.length) throw Error(JSON.stringify({ failedReads }));
  return {
    completedBytesSurviveLostReply: true,
    refreshRequiresExplicitPlacement: true,
    manualPlacementUsesOriginalIdentity: true,
