...
const directory = resolve("output/playwright/2026-09-12-canvas-complete-fit");
...
const ep = await f.ok("POST", `${f.path}/episodes`, {title:"全览技术验证",position:0,status:"active"},await f.next());
const cases=[];
for(const [index,count] of [2,2000].entries()) {
