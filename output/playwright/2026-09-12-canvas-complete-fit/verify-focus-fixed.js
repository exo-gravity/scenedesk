async (page) => {
 ...
 await page.getByRole('button',{name:'适应内容',exact:true}).click(); await page.waitForTimeout(100);if(!(await geometry()).allInside)throw Error('toolbar fit regression'); await page.getByRole('button',{name:'定位当前内容',exact:true}).click();
 await page.getByRole('button',{name:'定位当前内容',exact:true}).scrollIntoViewIfNeeded();
 await page.screenshot({path:'output/playwright/2026-09-12-canvas-complete-fit/multi-focus-fixed-1512.png',animations:'disabled'});
 ...
 await page.screenshot({path:'output/playwright/2026-09-12-canvas-complete-fit/list-fixed-390.png',animations:'disabled'});
