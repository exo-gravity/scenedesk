/** Synthetic isolated E2E: actual API/PG/MinIO/upload processing; local technical discussion/prompt/script-analysis/video fixture executors only; zero paid/network provider calls. */
...
const port = Number(process.env.CREATIVE_PORT ?? "4347");
const uiPending = process.env.CREATIVE_UI_PENDING === "1";
assert(
...
const builds = {
  ...
  after: await buildInput(
    ...
    uiPending ? undefined : "apps/web/dist",
  ),
};
const phase =
...
const evidence = resolve(
  "output/playwright/2026-09-15-assistant-conversation",
  runId,
);
await mkdir(directory, { mode: 0o700, recursive: true });
await mkdir(evidence, { recursive: true });
const distControl = resolve(directory, "dist.json");
const capabilityReadControl = resolve(directory, "capability-read-mode.json");
await writeFile(capabilityReadControl, JSON.stringify({ mode: "normal" }), {
  mode: 0o600,
  flag: "wx",
});
await writeFile(distControl, JSON.stringify(initialBuild ? { ...initialBuild, phase } : { phase, pending: true }), {
  mode: 0o600,
  flag: "wx",
});
await writeFile(
...
async function start() {
  ...
  const assistantCapabilityId = randomUUID(),
    scriptCapabilityId = randomUUID(),
    targetCapabilityId = randomUUID(),
    imageProfileCapabilityId = randomUUID(),
    imageFixtureCapabilityId = randomUUID();
  const extraTargetIds = Array.from({ length: 20 }, () => randomUUID());
  for (const [id, purpose, mode] of [
    [assistantCapabilityId, "creative_assistance", "fixture"],
    [scriptCapabilityId, "script_analysis", "structured_text"],
  ])
    await f.admin.query(
      ...
      [
        ...
        connectionVersionId,
        {
          ...
          mode,
          supportedPurposes: purpose === "script_analysis" ? [] : [
            "composition",
            "identity",
            "look",
            "location",
    ...
    adapters: [
      createAssistanceFixture(connectionVersionId, async (submission) => {
        if (submission.input.purpose === "creative_assistance")
          localAssistantCalls++;
        else if (submission.input.purpose === "script_analysis")
          localScriptCalls++;
        else if (submission.input.purpose === "video" && fixtureVideoOutput)
          localVideoCalls++;
        else
          throw Error(
            "Only explicitly configured local text/script/video fixtures may run",
          );
        return {
          kind: "completed",
          correlation: submission.attemptId,
          output:
            ["creative_assistance", "script_analysis"].includes(submission.input.purpose)
  ...
  // Synthetic options only: preserve the original video identity/default and
  // its executor. These additions exercise menus and fixed target selection.
  const extraModelOptions = [
    {
      id: imageProfileCapabilityId,
      purpose: "image",
      mode: "target_profile_fixture",
      modelVersion: "隔离合成 · 图片提示目标（不执行媒体）",
    },
    {
      id: imageFixtureCapabilityId,
      purpose: "image",
      mode: "image_fixture_v1",
      modelVersion: "隔离合成 · 图片技术模型（选项核验）",
    },
    ...extraTargetIds.map((id, index) => ({
      id,
      purpose: "video",
      mode: "target_profile_fixture",
  ...
  const targetPurposes = [
    "composition", "identity", "look", "location", "action", "style", "prop",
  ];
  for (const option of extraModelOptions)
    await f.admin.query(
      `INSERT INTO "${f.schema}".generation_capabilities(id,tenant_id,connection_id,connection_version_id,revision,definition,execution_mode,enabled,max_inflight,max_daily_jobs) VALUES($1,$2,$3,$4,1,$5,'test_fixture',true,2,100)`,
      [
        option.id,
        ...
        connectionVersionId,
        {
          ...
          supportedPurposes: targetPurposes,
          maxReferences: 20,
          inputRules: [{
            kind: "image",
            purposes: targetPurposes,
            mimeTypes: ["image/png", "image/jpeg"],
            minCount: 0,
            maxCount: 20,
            maxBytes: 16 * 1024 * 1024,
          }],
          allowedResolutions: option.purpose === "image"
            ? ["1024x1024", "1536x1024"]
            : [fixtureResolution],
          allowedAspectRatios: option.purpose === "image" ? ["1:1", "3:2"] : ["9:16"],
          ...(option.purpose === "video" ? {
            minDurationSeconds: fixtureDurationSeconds,
            maxDurationSeconds: fixtureDurationSeconds,
            audioOutput: false,
          } : {}),
          notes: "仅限隔离合成数据的模型菜单与固定计划选择验证；没有真实模型或付费调用，本夹具未接入新增图片执行器。",
        },
      ],
    );
  ...
  const fixture = {
    ...
    providerCalls: 0,
    generationExecutorStarted: "local_discussion_prompt_script_and_prebuilt_video_fixture_only",
    assistantCapabilityId,
    scriptCapabilityId,
    targetCapabilityId,
    imageProfileCapabilityId,
    imageFixtureCapabilityId,
    extraTargetIds,
    extraModelOptions: extraModelOptions.map((option) => ({
      ...option,
      executionMode: "test_fixture",
      selectionVerificationOnly: true,
    })),
  };
  ...
  app.addHook("onRequest", async (req, reply) => {
    if (req.method !== "GET" || req.url.split("?")[0] !== `${base}/capabilities`)
      return;
    let mode: unknown;
    try {
      const control = await readFile(capabilityReadControl, "utf8");
      if (Buffer.byteLength(control) > 4096) throw Error("Invalid fixture control");
      mode = JSON.parse(control).mode;
    } catch {
      mode = "error";
    }
    if (mode === "delay") await new Promise((done) => setTimeout(done, 3000));
    else if (mode !== "normal")
      return reply.code(503).header("cache-control", "no-store").send({
        code: "FIXTURE_CAPABILITIES_UNAVAILABLE",
        message: "隔离测试：模型列表暂时不可用，请重试。",
        requestId: req.id,
      });
  });
  ...
  app.get("/__fixture/requests", async () => ({ requests, localAssistantCalls, localScriptCalls, localVideoCalls, providerCalls: 0 }));
  app.get("/__fixture/build", async () => {
    const selected = JSON.parse(await readFile(distControl, "utf8"));
    if (selected.pending) return { pending: true, phase: selected.phase };
  ...
  app.get("/*", async (req, reply) => {
    ...
    const selected = JSON.parse(await readFile(distControl, "utf8"));
    if (selected.pending) return reply.code(503).type("text/plain").send("Isolated fixture API ready; immutable UI build not yet selected.");
    const dist = resolve(selected.path);
